...
local OTHER, LEVEL_RANGE, EMBLEM_SYMBOL, STATUS, CURRENCY, TRADE_SKILLS, FILTERS = OTHER, LEVEL_RANGE, EMBLEM_SYMBOL, STATUS, CURRENCY, TRADE_SKILLS, FILTERS
local COLOR, CLASS_COLORS, CUSTOM, COLOR_PICKER = COLOR, CLASS_COLORS, CUSTOM, COLOR_PICKER

-- GLOBALS: AceGUIWidgetLSMlists

local FISH_ICON = "|TInterface\\AddOns\\ElvUI\\Core\\Media\\Textures\\LocPlusFish.tga:24:24|t"
local LEVEL_ICON = "|TInterface\\AddOns\\ElvUI\\Core\\Media\\Textures\\LocPlusLevelUp.tga:24:24|t"

local displayOtherValues = {
	['NONE'] = L['None'],
	['RLEVEL'] = LEVEL_ICON.." "..LEVEL_RANGE,
	['PFISH'] = FISH_ICON.." "..L['Fishing'],
}
				...
				visibilityGroup = {
					...
					args = {
						...
						visibility = {
							...
							width = 'full',
							multiline = true,
							get = function() return E.db.locplus.visibility end,
							set = function(info, value)
								if value and value:match('[\n\r]') then
									value = value:gsub('[\n\r]','')
								end
								E.db.locplus.visibility = value;
								LP:UpdateVisibility()
							end,
						},
					},
...
-- Rename the LocationPlus datatext panels in the DataTexts > Panels options
do
	local panels = E.Options.args.datatexts.args.panels.args
	if panels.LocPlusLeftDT then
		panels.LocPlusLeftDT.name = L['LocationPlus Left Panel']
		panels.LocPlusLeftDT.order = 1101
	end
	if panels.LocPlusRightDT then
		panels.LocPlusRightDT.name = L['LocationPlus Right Panel']
		panels.LocPlusRightDT.order = 1102
	end
end
