...
E.PopupDialogs['DT_Panel_Add'] = {
	...
	button2 = CANCEL,
	OnAccept = function()
		local s = E.global.dtbarsSetup
		DB:InsertPanel(s.name, s.slots, s.growth, s.width, s.transparent, s.anchor, s.point, s.x, s.y, s.strata, s.hide, s.border)
		ReloadUI()
	end,
	timeout = 0,
	whileDead = 1,
...
-- Overwrite DT panel functions to support DTBars2 vertical growth and extra point locations
...
function DT:GetDataPanelPoint(panel, i, numPoints)
	...
	elseif numPoints == 2 then
		if i == 1 then return (vert and 'TOP' or 'LEFT'), panel, (vert and 'TOP' or 'LEFT'), vert and 0 or 4, vert and -4 or 0
		elseif i == 2 then return (vert and 'BOTTOM' or 'RIGHT'), panel, (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or -4, vert and 4 or 0 end
	elseif numPoints == 3 then
		if i == 3 then return 'CENTER', panel, 'CENTER'
		elseif i == 1 then return (vert and 'BOTTOM' or 'RIGHT'), panel.dataPanels['middle'], (vert and 'TOP' or 'LEFT'), vert and 0 or -4, vert and 4 or 0
		elseif i == 2 then return (vert and 'TOP' or 'LEFT'), panel.dataPanels['middle'], (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or 4, vert and -4 or 0 end
	elseif numPoints == 4 then
		if i == 1 then return (vert and 'BOTTOM' or 'RIGHT'), panel, 'CENTER', 0, vert and 2 or 0
		elseif i == 2 then return (vert and 'TOP' or 'LEFT'), panel, 'CENTER', 0, vert and -2 or 0
		elseif i == 4 then return (vert and 'BOTTOM' or 'RIGHT'), panel.dataPanels['left'], (vert and 'TOP' or 'LEFT'), vert and 0 or -4, vert and 2 or 0
		elseif i == 5 then return (vert and 'TOP' or 'LEFT'), panel.dataPanels['right'], (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or 4, vert and -2 or 0 end
	elseif numPoints == 5 then
		if i == 3 then return 'CENTER', panel, 'CENTER'
		elseif i == 1 then return (vert and 'BOTTOM' or 'RIGHT'), panel.dataPanels['middle'], (vert and 'TOP' or 'LEFT'), vert and 0 or -4, vert and 4 or 0
		elseif i == 2 then return (vert and 'TOP' or 'LEFT'), panel.dataPanels['middle'], (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or 4, vert and -4 or 0
		elseif i == 4 then return (vert and 'BOTTOM' or 'RIGHT'), panel.dataPanels['left'], (vert and 'TOP' or 'LEFT'), vert and 0 or -4, vert and 4 or 0
		elseif i == 5 then return (vert and 'TOP' or 'LEFT'), panel.dataPanels['right'], (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or 4, vert and -4 or 0 end
	else
		if i == 3 then return 'CENTER', panel, 'CENTER'
		elseif i == 1 then return (vert and 'BOTTOM' or 'RIGHT'), panel.dataPanels['middle'], (vert and 'TOP' or 'LEFT'), vert and 0 or -4, vert and 4 or 0
		elseif i == 2 then return (vert and 'TOP' or 'LEFT'), panel.dataPanels['middle'], (vert and 'BOTTOM' or 'RIGHT'), vert and 0 or 4, vert and -4 or 0 end
...
function DT:LoadDataTexts()
	...
	if DT.ForceHideBGStats then DT.ForceHideBGStats = nil end
end
...
function DB:ProfileHandle(name, data)
	...
	local slotCount = data.slots
	local slotKeys = { 'left', 'right', 'middle', 'farleft', 'farright' }
	local slotDefaults = {}
	for i = 1, slotCount do slotDefaults[slotKeys[i]] = '' end

	if not P.datatexts.panels[name] then P.datatexts.panels[name] = slotDefaults end
	if not E.db.datatexts.panels[name] then E.db.datatexts.panels[name] = {} end
	for i = 1, slotCount do
		if not E.db.datatexts.panels[name][slotKeys[i]] then E.db.datatexts.panels[name][slotKeys[i]] = '' end
	end
end
...
function DB:ChangeSlots(panel)
	for _, data in pairs(ElvDB.profiles) do
		if (data.datatexts and data.datatexts.panels) and data.datatexts.panels[panel] then
			data.datatexts.panels[panel] = nil
		end
	end
	ReloadUI()
end
...
function DB:OnEvent(event, unit)
	...
	for name in pairs(E.global.dtbars) do
		if name then
			local db = E.db.dtbars[name]
...
function DB:Initialize()
	...
	DT:LoadDataTexts()
	EP:RegisterPlugin('ElvUI_DTBars2', DB.GetOptions)
end
