	...
	E.Options.args.visualProcs = {
		...
		args = {
			overlayTest = {
				...
				func = function() VP:ToggleTestFrame() end,
			...
			buttonGlow = {
				...
				type = 'toggle',
				name = L["Button Glow"],
				get = function() return E.db.visualProcs.buttonGlow.enable end,
			...
			overlay = {
				...
				get = function() return E.db.visualProcs.overlay.enable end,
				set = function(_, value)
			...
			overlayScale = {
				...
				get = function() return E.db.visualProcs.overlay.scale end,
				set = function(_, value)
			...
			disableSound = {
				...
				get = function() return E.db.visualProcs.overlay.disableSound end,
				set = function(_, value)
...
local function OverlayGlowShow(self, spellID)
	if self:GetSpellId() == spellID then LBP:ShowOverlayGlow(self) end
end

local function OverlayGlowHide(self, spellID)
	if self:GetSpellId() == spellID then LBP:HideOverlayGlow(self) end
end
...
local function UpdateOverlayGlow(self)
	...
	if self:HasAction() then
		...
		if spellID then LBP:ChangeAction(self._state_action, spellID) end
	else
...
function VP:ToggleTestFrame()
	...
	if self.overlayFrame.test then
		...
		for _, data in ipairs(LBP_Data.OverlayTextures[LBP.playerClass]) do
			...
			if foundTop and foundLeft then break end
		end
...
function VP:UpdateState()
	if LBP.disableOverlay and LBP.disableButtonGlow then
		if LBP:isEnabled() then LBP:Disable() end
	else
		if not LBP:isEnabled() then LBP:Enable() end
	end
end

function VP:Initialize()
	LBP.mediaPath = 'Interface\\AddOns\\ElvUI\\Core\\Modules\\ActionBars\\LibBlizzardProcs\\textures\\'
	...
	EP:RegisterPlugin('ElvUI_VisualProcs', GetOptions)
end
